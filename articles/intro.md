...
## Cheat sheet: storage, speed, accuracy (extensible tibble)

``` r
if (requireNamespace("tibble", quietly = TRUE)) {
  tibble::tribble(
    ~family,              ~stored,                                 ~implicit,                          ~accuracy,                       ~speed_notes,
    "slepian_space",      "basis (time x k), loadings (vox x k)",   "none (explicit)",                  "exact on span; orthogonal per cluster", "fast; lift per cluster; sparse loadings",
    "hrbf",               "basis (time x atoms), loadings (vox x atoms)", "none (explicit)",            "improves as atoms↑ (levels)",       "moderate; dense atoms; no graph build",
    "wavelet_active",     "coeff (time x vox)",                     "decoder (active pencils)",         "exact (biorthogonal 5/3)",        "very fast; mask-aware lifting",
    "bspline_hrbf_st",    "core (kt x ks), B_t, L_s",               "decoder (separable)",              "approx -> better with atoms",    "fast; separable; HRBF once",
    "slepian_st",         "core (kt x ks), B_t, L_s",               "decoder (separable)",              "exact on span of bases",          "fast; sparse spatial loadings",
