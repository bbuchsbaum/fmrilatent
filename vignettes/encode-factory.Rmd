...
```{r pca-space, eval = FALSE}
# Define parcels (4 equal regions) and build a ClusterReduction
...
```{r parcel-template, eval = FALSE}
# Wrap the parcellation as a ClusteredNeuroVol (reusing parcel_map from above)
atlas <- neuroim2::ClusteredNeuroVol(mask_vol, parcel_map)

# Build template once (geometric basis — no data needed)
...
You can also build a shared PCA template from group data:
...
| `"parcel_space"` | `spec_space_parcel()` (pass `template =`) |
